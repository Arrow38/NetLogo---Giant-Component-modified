...
to go
  ...
  if not all? turtles [ color = red or color = green  ] [ layout ]
  tick
end
...
to greenturtle
  ask turtles [set Disconnected? false]
  set compteur 0
  while [compteur < n_disconnected]
  [
    let d one-of turtles with [ not Disconnected? ]
    ask d [disconnect]
...
to add-edge
  ...
  ask node1 [
    ...
    ;;if [color node2 red] or [color node1 red]
